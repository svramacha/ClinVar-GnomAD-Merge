...
filter_path <- final_combined_data_2 %>%
  filter(FILTER.y == "PASS", FAFmax_faf95_max_joint > faf_cutoff) %>%
  dplyr::select(merge, pathogenic)  # Replace variant_id_column with your actual variant identifier column
...
d_all <- d_all %>%
  ...
  dplyr::select(-pathogenic.x, -pathogenic.y)  # Remove the old columns if needed

# Sneha's - trying to add the PHRED 
...
model_estimates_weight <- list(params = summary(model)$coef[,1],
  ...
  for (i in 1:n_imputations) {
    ...
    # Ensure PHRED exists
    d_imp <- d_imp %>%
      mutate(PHRED = coalesce(PHRED.y, PHRED.x))  # Use PHRED.y first, then PHRED.x
    
    # Drop old PHRED columns
    d_imp <- d_imp %>%
      dplyr::select(-PHRED.x, -PHRED.y)
    ...
    # Ensure PHRED is present before running the model
    if (!"PHRED" %in% colnames(d_imp)) {
      stop("PHRED column missing in d_imp")
    }
